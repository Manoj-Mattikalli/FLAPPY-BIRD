...
pass_pipe = False
level = 1
level_up_threshold = 5  # Points required to level up
player_name = ""
            ...
            if len(pipe_group) > 0:
                ...
                if pass_pipe:
                    if bird_group.sprites()[0].rect.left > pipe_group.sprites()[0].rect.right:
                        score += 1
                        pass_pipe = False
                        if score % level_up_threshold == 0:  # Level up every threshold
                            level += 1
                            scroll_speed += 1

            draw_text(f"Score: {score}", font, white, 30, 20)
            draw_text(f"Level: {level}", font, white, screen_width - 200, 20)

        if game_over:
            # Update high score based on the current game score
            high_score = update_score(player_name, score)

            text_y_start = screen_height - 350  # Start displaying from 150 pixels above the bottom

            draw_text("Game Over!", font, white, 30, text_y_start - 10)
            draw_text(f"Score: {score}", small_font, black, 30, text_y_start + 60)
            draw_text(f"High Score: {high_score}", small_font, black, 30, text_y_start + 120)
            draw_text(f"Level: {level}", small_font, black, 30, text_y_start + 170)
